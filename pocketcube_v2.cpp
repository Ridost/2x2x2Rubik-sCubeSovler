...
vector<vector<string>>PruneTable;
string ShortestPath = "                                       ";
...
void *Solve(void * arg){
	...
	while (1)
	{
		...
		if(tans.length()==20) pthread_exit(0);
		//cout<<tans<<endl;
        ...
        if(tans.length()==18){  //find solition from prune_table
            for(int i=0;i<=4;i++){
                pthread_rwlock_rdlock(&rwlock);
                if((tans.length()+i*2)>=ShortestPath.length())    break;
                pthread_rwlock_unlock(&rwlock);
                for(int j=0;j<PruneTable[i].size();j++){
                    if(status==PruneTable[i][j].substr(12,24)){   //find solution
                        int pathlen = PruneTable[i][j].find("  ");
                        tans+=" "+PruneTable[i][j].substr(0,pathlen);
                        //cout<<tans<<endl;
                        pthread_mutex_lock(&mutex);
                        if(ShortestPath.length()>tans.length()){
                            ShortestPath=tans;
                            cout<<"Solution Update "<<ShortestPath.length()/2<<":"<<ShortestPath<<endl;
                        }
                        pthread_mutex_unlock(&mutex);
                    }
                }
            }
            // Error404: Solution Not Found
        }
		for(int i=0;i<6;i++)
		{
			//cout<<"i="<<i<<endl;
...
int main(){
    ...
    for(int  i=1;i<=6;i++){
        ...
        while(getline(fin,inputStr)){
            temp.push_back(inputStr);
        }
        PruneTable.push_back(temp);
        fin.close();
        cout<<"PruneTable"<<i<<" : Loaded"<<endl;
    }
    //cout<<PruneTable[5][5]<<endl;
